...
var router = new director.http.Router().configure({ recurse: null }); // 'backward'
var Qs = require('qs');
var rx = require('rx'),
    rxo = rx.Observable,
...
var rxSpec = rxParser(path.join(process.cwd(), 'swagger.json'))
...
function setupPath(x) {
    ...
    _.forEach(x.methods, function(data, method) {
        ...
        router.on(method, verpath_params, function() {
            ...
            try {
              routes[handler](context, makeEvent(_this.req, _this.res, handler), RELOAD);
            } catch(e) {
            //if(false) {
              //var err = new Error();
            ...
            if (RELOAD) delete require.cache[require.resolve(routesPath)];
            return false;
        });
    });
