...
// { port: 3000, noreload: true, editor: false }
Lambdapi.prototype.run = function(program) {
  var _this = this;
  var port = program.port;
  var handler = program.handler || null;

  var server = require(server_path);
  if(program.editor) server.startWithEditor(port, handler, function() {
    // onChange spec
    try {
      _this.setup('swagger.json', program);
    } catch(e) {
      console.log("Save fail- possible spec fail", e);
    }
  });
  else server.start(port, !program.noreload, handler);
};
...
Lambdapi.prototype.setup = function(file, options) {
  ...
  // Try the local project's .env.example first
  // TODO: refactor to be cleaner using file exists
  try {
    copyFileIfNew(path.join(process.cwd(), '.env.example'), 
  ...
  try {
      copyFileIfNew(path.join(process.cwd(), 'deploy.env.example'), 
    ...
    return function (text, render) {
      ...
      try {
        ...
        if(options.commentignore) console.log("controllers/" + parse[0] + " already made, skipping.");
          ...
          fs.writeFileSync(file, newComments.toString() + controllerBody);
       }
      }
      return "";
    };
...
*/
  ...
  fs.mkdir(controller_dir, function(err, resp) {
    ...
    var source;
   //try {
      source = CodeGen.getCustomCode(source_config);
    /*} catch(e) {
      console.log("Swagger spec parse error");
      console.log(e);
      return;
    }*/
